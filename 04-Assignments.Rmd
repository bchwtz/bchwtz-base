...
**Bearbeitungsmodus:** Abgaben können grundsäztlich in Einzelarbeit (Modus: Individuell) erarbeitet werden oder durch mehrere Autoren erstellt werden (Modus: Gruppe). Bei Gruppenarbeiten müssen alle beteiligten Autoren inklusive Matrikelnummer im Dokument benannt sein. Die Kommunikation  mit dem Dozenten übernimmt ein/e durch alle Mitglieder der Gruppe gewählte/r Gruppensprecher/in, die/der die Abgabe auch formal einreicht. Die Abgabe ist durch ein *Statement of Contribution* zu ergänzen, das Art und Anteil eines jeden Gruppenmitgliedes an der Abgabe listet. 
...
- **MECE:** Der Aufbau der einzelnen Botschaften muss dem MECE-Prinzip folgen (mutually exclusive and collectively exhaustive).
...
Ein beispielhafter Evaluationsbogen für akademische Poster ist hier zu finden:

- [Beispiel Evaluationsbogen](./data/bchwtz-posterevaluation.pdf)
- [Word Evaluationsbogen](./data/bchwtz-posterevaluation.docx)

## Dataset

Jeder einzureichende Datensatz besteht mindestens aus den folgenden **drei** Bestandteilen:

1) **Datendatei:** Die eigentliche Datendatei muss in einer maschinenlesbaren, dokumentierten und aufbereiteten Form bereitgestellt werden.  Dabei ist nicht relevant um welche genaue Art der Datei (.csv, .xlsx, .json, ...) es sich handelt, solange diese mit *üblichen* verfügbaren Mittel einlesbar ist. Sollte es sich bei den Rohdaten um ein selten verwendetes oder **exotisches** Dateiformat (z.B. Datenbankdump) handeln kann die Abgabe der Rohdaten durch ein Skript bzw. Programm ergänzt werden, das das Aufbereiten der Daten nachvollziehbar erläutert und eine (idealerweise tabellarische) geeignete Datei zur weiteren Analyse erzeugt. Das bereitstellen eines Links oder der Verweis auf ein Repository ist ungenügend.

2) **Variablenbeschreibung:** Alle in der Datendatei enthaltenen Variablen müssen hinsichtlich ihres Inhaltes und ihrer Bedeutung nachvollziehbar beschrieben sein. Eine vollständig korrekte Beschreibung beinhaltet zudem etwaige Erfassungs- bzw. Geltungszeiträume der vermerkten Größen und benennt die zugehörigen Einheiten.

3) **Kurzskizze des Untersuchungsvorhaben:** Zur Prüfung der Eignung des eingereichten Datensatzes ist dieser durch eine kurze Skizze des Analysevorhabens (in Form einer potenziellen Analysegleichung oder einer textuellen Skizze der hypothethisierten Abhängigkeiten) zu ergänzen. Die Kurskizze kann den Charakter eines kurzen, auf die eigentliche Datenanalyse beschränkten Exposées aufweisen.

Um alle Rechnungen und Interpretationen nachvollziehen zu können, muss jeder Datensatz dem Dozenten vollständig zugänglich gemacht werden. Sollten Sie einen Datensatz verwenden wollen, der nicht oder nur eingeschränkt  mit dem Dozenten geteilt werden kann, eigenet dieser sich nicht für Bearbeitung einer Abgabeaufgabe. Natürlich können Sie jeden verwendeten Datensatz anonymisieren, sofern die Interpretierbarkeit der Ergebnisse dadurch nicht eingeschränkt oder unterbunden wird. 
