...
- **\LaTeX:** Die Verwendung von reinem \LaTeX für die Erstellung von Abgaben ist ebenfalls zulässig, sofern sie die unangepasste `article` Klasse oder die Vorlage eines wissenschaftlichen Journals oder einer Konferenz (z.B. `elsarticle`) nutzen. Andere oder angepasste Varianten (auch Hochschulvorlagen) der vorgenannten Dokumentenklasse sind nicht zulässig. 

Die Verwendung von *Microsoft Word* oder ähnlichen Textverarbeitungsprogrammen ist **nicht** ohne weiteres zulässig und Bedarf der Rücksprache mit dem Prüfer und wird nur genehmigt, wenn die verwendete Vorlage Bedingung für die Einreichung bei einem wissenschaftlichem Outlet (z.B. International Conference on Information Systens) ist.

Die Bewertungsgrundlage für die Abgabe ist **ein** erzeugtes PDF Dokument, dass alle Elemente der Arbeit enthält. Alle Quelldateien (z.B. `*.tex`, `*.rmd`, `*.bib`) zur Erstellung dieses PDF Dokumentes müssen zwingend mit eingereicht werden. Die Abgabe kann durch eienn strukturierten und dokumentierten digitalen Anhang (z.B. längerer Code) ergänzt werden. Für einen digitalen Anhang ist im PDF Dokument ein Verezichnis mit Kurzbeschreibung der im digitalen Anhang enthaltenen Elemente einzufügen. Das zu bewertende PDF muss eine unterschriebene ehrenwörtliche Erklärung enthalten.

## Präsentation

Wissensvermittlung (K)
Ergebnisvorstellung (R)
